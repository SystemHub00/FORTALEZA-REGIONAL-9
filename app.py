...
COURSE_CATALOG = [
    ...
    # NOVO
    {"id": "5", "nome": "26/EXTC 004 - EXTENS\u00c3O DE C\u00cdLIOS"},
]
# =============================================================================
# 3. HOR\u00c1RIOS
# =============================================================================
...
TURMA_OPTIONS = [
    ...
    # Intelig\u00eancia Artificial \u2014 Polo 1 \u2014 Ter/Qui 15h\u201317h
    {
        "id": "4",
        "curso_id": "4",
        "local_id": "1",
    ...
    # NOVO \u2014 Extens\u00e3o de C\u00edlios \u2014 Institui\u00e7\u00e3o Coletivo Olhando Pra Frente \u2014 Seg a Sex 08h\u201312h
    {
        "id": "5",
        "curso_id": "5",
        "local_id": "3",
        "turma_codigo": "26/EXTC-004",
        "agenda_id": "5",
        "periodo_id": "5",
        "encerramento_id": "5",
        "endereco_id": "2",
...
<head>
    ...
    <style>
        :root {
            ...
            --cor-principal-escura: #5a0000;
            --cor-clara:            #fbe8e8;
            --cor-texto:            #5a0000;
            --cor-borda:            #e2a3a3;
            --sombra-card:          0 18px 55px rgba(139,0,0,0.18);
        }
        * { box-sizing: border-box; }
        ...
        body {
            min-height: 100vh;
            background:
                radial-gradient(circle at top left, rgba(139,0,0,0.14), transparent 34%),
                radial-gradient(circle at top right, rgba(255,220,220,0.82), transparent 32%),
                linear-gradient(135deg, #fff5f5 0%, #fff 42%, #fbe0e0 100%);
            color: var(--cor-texto);
        ...
        .wizard-panel[data-step="escolher"] #local_display, .wizard-panel[data-step="escolher"] #dias_aula,
                    ...
                    <div class="hero-grid">
                        <div class="hero-card">
                            ...
                            <p style="margin:10px 0 0; color:#8b0000; font-size:0.9rem; font-weight:700; line-height:1.5;">
                                Instituto Al&#233;m dos Olhos - Polo 1<br>
                            ...
                            <p class="hero-subtitle">
                                ...
                                primeiro passo para transformar sua vida profissional.
                            </p>
                            <div class="hero-highlights">
                                ...
                                <div class="hero-highlight">
                                    <strong>FORTALEZA - REGIONAL IX</strong>
                    ...
                    <div class="step-card">
                        ...
                        <div class="step-grid step-grid--stacked">
                            ...
                            <div class="form-group full" id="local-group" style="display:none;">
                                ...
                                <select id="local_id_select"><option value="">Selecione um local</option></select>
                            ...
                            <div class="form-group full" id="turma-group" style="display:none;">
                                ...
                                <select id="opcao_id_select"><option value="">Selecione um hor&#225;rio</option></select>
                ...
                <section class="wizard-panel" data-step="revisao">
                    <div class="step-card">
                        ...
                        <div class="review-layout">
                            ...
                            <div class="review-box full">
                                <div class="review-info-text" style="margin-bottom:10px; color:#8b0000; font-size:0.98rem; text-align:left;">
                                    <strong>Elegibilidade:</strong> Este curso &#233; destinado a pessoas interessadas em qualifica&#231;&#227;o profissional.
                                </div>
                                <label class="review-check" for="confirma_dados">
                                    ...
                                    <span>
                                        ...
                                        Tamb&#233;m autorizo o uso da minha imagem para divulga&#231;&#227;o nos canais de comunica&#231;&#227;o e redes sociais do projeto e da Prefeitura.
                                    </span>
                                </label>
                                <div class="review-info-text" style="margin-top:10px;">
                                    <strong>Ao confirmar voc&#234; declara a ci&#234;ncia de que:</strong>
                                    <ul><li>O curso &#233; totalmente gratuito</li></ul>
                                </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function aplicarOpcao(opcaoId){
                ...
                if(!op){opcaoIdInput.value='';localIdInput.value='';localInput.value='';if(localDisplay)localDisplay.value='';cursoInput.value='';turmaInput.value='';diasAulaInput.value='';horarioInput.value='';dataInicioInput.value='';encerramentoInput.value='';enderecoInput.value='';mostrarInfoCurso(false);return;}
            ...
            function atualizarLocaisPorCurso(cursoId,selectedLocalId,selectedOpcaoId){
                var turmasDoCurso=courseOptions.filter(function(o){return String(o.curso_id)===String(cursoId||'');});
                var localIdsUsados=[],locaisUnicos=[];
                turmasDoCurso.forEach(function(o){if(localIdsUsados.indexOf(o.local_id)===-1){localIdsUsados.push(o.local_id);var l=localOptions.find(function(x){return String(x.id)===String(o.local_id);});if(l)locaisUnicos.push(l);}});
                localSelectEl.innerHTML='';opcaoSelectEl.innerHTML='';turmaGroup.style.display='none';aplicarOpcao('');
                if(locaisUnicos.length===0){localGroup.style.display='none';return;}
                localGroup.style.display='';
                var ph=document.createElement('option');ph.value='';ph.textContent='Selecione um local';localSelectEl.appendChild(ph);
                locaisUnicos.forEach(function(loc){var opt=document.createElement('option');opt.value=loc.id;opt.textContent=loc.nome;if(String(loc.id)===String(selectedLocalId||''))opt.selected=true;localSelectEl.appendChild(opt);});
                if(locaisUnicos.length===1&&!selectedLocalId){localSelectEl.value=locaisUnicos[0].id;atualizarHorariosPorLocal(cursoId,locaisUnicos[0].id,selectedOpcaoId);return;}
                if(selectedLocalId&&localIdsUsados.indexOf(String(selectedLocalId))!==-1){atualizarHorariosPorLocal(cursoId,selectedLocalId,selectedOpcaoId);}
            }
            function atualizarHorariosPorLocal(cursoId,localId,selectedOpcaoId){
                var turmas=courseOptions.filter(function(o){return String(o.curso_id)===String(cursoId||'')&&String(o.local_id)===String(localId||'');});
                opcaoSelectEl.innerHTML='';
                if(turmas.length===0){turmaGroup.style.display='none';aplicarOpcao('');return;}
                turmaGroup.style.display='';
                var ph=document.createElement('option');ph.value='';ph.textContent='Selecione um hor\u00e1rio';opcaoSelectEl.appendChild(ph);
                turmas.forEach(function(op){var opt=document.createElement('option');opt.value=op.id;opt.textContent=op.dias_aula+' | '+op.horario;if(String(op.id)===String(selectedOpcaoId||''))opt.selected=true;opcaoSelectEl.appendChild(opt);});
                if(selectedOpcaoId&&turmas.some(function(o){return String(o.id)===String(selectedOpcaoId);})){aplicarOpcao(selectedOpcaoId);}
                else if(turmas.length===1){opcaoSelectEl.value=turmas[0].id;aplicarOpcao(turmas[0].id);}
                else{aplicarOpcao('');}
            }
            ...
            benefitsSliders.forEach(initBenefitsSlider);syncReview();
...
SUPABASE_API_KEY = os.environ.get(
    "SUPABASE_API_KEY",
    "GGbPIn53S1IH1F4i6uBS5Ftoh1fNYk49",
)
